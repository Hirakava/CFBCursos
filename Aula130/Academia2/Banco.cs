...
namespace Academia2
{
    class Banco
    {
        ...
        public static DataTable dql(string sql)  //data query language (CONSULTA - SELECT)
        {
        ...
        public static void dml(string q, string msgOK=null,string msgERRO=null)  //Data Manipulation Language(INSERT, DELETE, UPDATE )
        {
            ...
            try
            {
                var vcon = ConexaoBanco();
                var cmd = vcon.CreateCommand();
                cmd.CommandText = q;     //"UPDATE tb_usuarios SET T_NOMEUSUARIO='" + u.nome + "', T_USERNAME='" + u.username + "', T_SENHAUSUARIO='" + u.senha + "', T_STATUSUSUARIO='" + u.status + "', N_NIVELUSUARIO=" + u.nivel + " WHERE N_IDUSUARIO=" + u.id;
                da = new SQLiteDataAdapter(cmd.CommandText, vcon);
                cmd.ExecuteNonQuery();
                vcon.Close();
                if (msgOK != null)
